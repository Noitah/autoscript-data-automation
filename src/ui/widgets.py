...
class LogFrame(ttk.LabelFrame):
    ...
    def __init__(self, parent, title: str = "Log de Execucao", height: int = 10, **kwargs):
        ...
        ttk.Button(btn_frame, text="Limpar Log", command=self.clear).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Copiar", command=self.copy_all).pack(side=tk.LEFT, padx=5)
    ...
    def append(self, message: str) -> None:
        def _insert():
            ...
            if message.startswith("\u2713") or "sucesso" in message.lower() or "conclu" in message.lower():
                tag = "success"
            elif message.startswith("\u2717") or "erro" in message.lower():
                tag = "error"
            else:
                tag = "info"
...
class StatusBar(ttk.Frame):
    ...
    def set_running(self, message: str = "Executando...") -> None:
        self.var.set(message)
        self.progress.grid()
        self.progress.start(15)
    
    def set_error(self, message: str = "Erro") -> None:
        self.var.set(f"Erro: {message}")
        self.progress.stop()
        self.progress.grid_remove()
    
    def set_success(self, message: str = "Concluido") -> None:
        self.var.set(f"Concluido: {message}")
        self.progress.stop()
        self.progress.grid_remove()
...
class FileField(ttk.Frame):
    ...
    def __init__(self, parent, label: str, browse_command, is_folder: bool = False, **kwargs):
        ...
        ttk.Button(self, text="Procurar...", command=self._on_browse, width=12).grid(row=0, column=2)
